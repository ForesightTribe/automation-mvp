...
|---|---|---|---|
| **Auth** | ⚠️ *Superseded 2026-08-04.* Blinkit logins are now **browserless REST** via the `platform_auth/` package — see [platform-auth.md](platform-auth.md) | `cli auth login zepto -t <uuid>` | Zepto is already registered `wired=False` in `platform_auth/registry.py` (fails fast until implemented) with `needs_password=True` — unlike Blinkit it uses **email + password**, and `platform_credentials` stores the password Fernet-encrypted. To wire it: add `marketplaces/zepto/`, a rule in `mail_rules.py` (its subjects are still guesses), and flip `wired=True`. Recon the login endpoints first — do NOT assume a browser is needed, and do not assume Firebase |
