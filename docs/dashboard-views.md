...
| ----------------------------------------------------------------------------------------------- | ----------------- | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| Headline KPIs + growth (rev, units, SKUs, spend, **RoAS**, impressions, **visibility**, **avg rank**) | KPI strip         | `blinkit_seller_sales` + `blinkit_ad_campaign_daily`(budget, ad_sales) + `search_results`(brand_sov, brand_rank via own watchlist)                    | `analytics/overview` **[B]** (PeriodDep + `marketplaces`, deltas; RoAS = ad_sales÷spend)  |
...
| Revenue/units trend                                                                             | Trend chart       | `blinkit_seller_sales`(date, mrp_value, qty_sold)                                                                      | `analytics/revenue` **[B]** (PeriodDep + `marketplaces`)                        |
