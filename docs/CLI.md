...
they can see all of its clients' data â€” `member` by default, or `--admin`.
...
Platform sessions â€” logging Foresight **into** Blinkit and Zepto. (Not app-user auth;
...
Every command below is **generic over the registry** â€” the platform is an argument, not a
command. Adding Zepto needed no CLI change at all.

### First time for a tenant â€” store the credentials
...
Blinkit is passwordless (magic link / OTP), so no password is stored. **Zepto is not** â€”
...
the session â€” about 15â€“20 seconds, no typing. Add `--manual` to paste the link/OTP
...
âš ï¸ **Refresh does not apply to Zepto.** Zepto exposes no refresh/rotate endpoint â€”
`refreshToken` is null in every response â€” so `auth refresh zepto` does nothing and
...
it is re-logged-in daily instead â€” see the `auth.login` schedule below.

âš ï¸ **Zepto allows ONE session per user.** A new login server-side revokes the previous
one, in both directions: our login evicts a human's dashboard, and their login kills our
session mid-run. So a Zepto session can be dead long before `expires_at` â€” `probe` is the
...
platform right now (one API call, no browser). A session can read `active` and be dead â€”
that gap is what let the seller scrape fail silently for weeks. When in doubt, `probe`.

### You rarely need any of this

Scrapers call `ensure()` internally: load â†’ probe â†’ refresh â†’ re-login, doing the least
...
dashboard, midnight is when that costs a human the least. `--catchup` matters here â€” a
...
not count) â€” it would otherwise hammer a login endpoint from one datacenter IP and bury a
...
`jobs.error='auth_expired'` â€” so auth failures are filterable in Cloud Logging rather than
...
```bash
# Daily run â€” defaults to the last 7 days (catches late metric revisions)
python -m cli scrape blinkit --tenant <tenant_id>

# Backfill a window (one pass) â€” e.g. last 30 days
...
collections, and visibility plans. Cost â‰ˆ `1 + 2Â·N` API calls (N = campaign count),
each covering the whole `--from`/`--to` window â€” so a 30-day backfill is one pass,
not 30 runs.
...
| ---------------------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `blinkit_ad_campaign_daily`  | per campaign Ã— **day**                          | budget, impressions, ATC, qty sold, ad sales, RoAS â€” the metric **backbone**                                               |
| `blinkit_ad_campaigns`       | per campaign (metadata)                         | name, type, status, start/end, infinite â€” the catalog                                                                      |
| `blinkit_ad_campaign_detail` | per campaign Ã— keyword/asset (window aggregate) | keyword/recommendation: impressions, budget, CPM, direct/indirect ATC & sales, qty, new users, position, direct/total RoAS |
...
summing `blinkit_ad_campaign_daily` â€” there is no separate performance-summary
table. RoAS is always recomputed as `Î£ ad_sales Ã· Î£ budget` over the window.

---

### Blinkit seller â€” sales + PO + SOH

```bash
# Daily run â€” scrapes all three: sales (yesterday), PO, and stock on hand
...
**Sales** â€” scrapes day-by-day over the given range (default: yesterday). Each day upserted by `item_id + city + date`; re-running the same date updates in place.

**PO** â€” scrapes a rolling window of POs by issue date. Upserted by `po_number` so re-running updates state without duplicating. SKU line items fetched only for POs not already in the DB â€” first run is expensive, subsequent runs fetch only new POs.

**SOH** â€” scrapes today's stock levels per SKU per facility. Upserted by `item_id + facility_id + date`; re-running the same day updates in place.
...
### Blinkit seller â€” scorecard (fill rates)

```bash
# Standard run â€” fetches the most recently published week
python -m cli scrape blinkit-scorecard --tenant <tenant_id>

# Specific week â€” pass the Monday start date
...
Blinkit publishes each week's scorecard on the following Monday, so the default always fetches last week's data (current Monday âˆ’ 7 days). Intended to run once per week. Uses the same `blinkit_seller` session â€” no separate auth needed.
...
| Table                          | Data                                                                    |
| ------------------------------ | ----------------------------------------------------------------------- |
| `blinkit_scorecard_weekly`     | Overall fill rate, weighted fill rate, PO/GRN qty, GMV, rank â€” per week |
...
### Public product search (Blinkit) â€” no login required
...
`blinkit` â€” so every command line below works unchanged. **Blinkit is the only
...
**1. Configure â€” `cli sync`.** `config.xlsx` has three sheets:

| Sheet       | Columns                                                                                    | What it is                                                                                                                                                                                                                                                                 |
| ----------- | ------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `locations` | `mp, merchant_id, city, state, region, pincode, lat, lon, active, location_name, address`  | that marketplace's **express** store catalog (keyed on `mp` + `merchant_id`); `lat/lon` is the probe point. `mp` is optional, blank â†’ `blinkit`. Longtail/super_longtail hubs are NOT here â€” they are discovered from scrape responses. See [darkstores.md](darkstores.md) |
...
| `coverage`  | `mp, tenant, city`                                                                         | which cities a tenant scrapes on which marketplace (all that marketplace's stores in each listed city). `mp` blank â†’ `blinkit`                                                                                                                                             |

> âš ï¸ **`--prune` only deletes within the marketplaces the file mentions** â€” a
...
The two caps (own rows only; blank â†’ code default): **`keyword_cap`** bounds the
...
> âš ï¸ **Both public scrapes write to a local SQLite staging file, not to Postgres.**
> Push them afterwards with `cli scrape load` (see 2c). A scraped-but-unloaded file is
> the one new failure mode â€” `cli scrape staged` lists anything pending. Full rationale
> in [staging.md](staging.md).

**2a. Keyword scrape â€” `cli scrape public-run`.** Every category keyword Ã— covered
store â†’ SoV/rank + declared competitors. Stages rows destined for `search_snapshots` +
`search_listings`.
...
**2b. Targeted own-SKU scrape â€” `cli scrape public-skus`.** Searches the tenant's
**brand name** and paginates its whole catalog to `brand_cap`, own-brand only â†’
`sku_snapshots` (one row per product Ã— store, keyed on `product_id`). This
...
**2c. Push to Postgres â€” `cli scrape staged` / `load` / `discard`.**
...
2026-07-19   14:53   skus     2,059/2,059  35,802     1   ok Â· loaded     145345
2026-07-18   09:12   search     500/2,059     100   847   failed Â· â€¦      091203
```

Each file loads in **one all-or-nothing transaction** â€” a failure writes nothing, so
...
auto-skipped though â€” 500 of 2,059 stores is still 500 stores of real data, and only
you can judge that. Loaded files are kept (last 5 per tenant per kind) then pruned.

**Common behaviour (both scrapes).** Each fresh run is a new staging file â€” run
...
unreachable. `--workers N` runs N isolated browser contexts on one browser (~5â€“6 is a
good IP balance; drop to 3 if Cloudflare throttles) â€” note the scrape holds **no DB
...
single line with the reason â€” e.g. `HTTP 403 Â· non-JSON body (Cloudflare?)` (the
throttle signal â€” reduce `--workers`) vs `HTTP 400` (a genuinely bad store/keyword,
ignorable in small numbers).

**Ad-hoc single scrape â€” `cli scrape public`.** One keyword at one location for
...
| `search_listings`  | `public-run`  | Per product in the result page: brand, price, MRP, discount %, in-stock, inventory, position, `extra` (group_id, merchant_id, unit, categoryâ€¦) |
| `sku_snapshots`    | `public-skus` | Per (own product Ã— location Ã— scrape), keyed on `platform_product_id`: name, price, MRP, discount %, in-stock, inventory, rating, `is_combo`   |
...
**3. Map private â†” public â€” `cli sku-map`.** Bridges the private seller `item_id`
...
#   â†’ open the workbook, fill platform_product_id for any unmatched rows
...
## Explore â€” on-demand custom scrape â†’ Excel (agency-facing)
...
tables â€” it's for profiling a prospect or a one-off deep-dive. Each run logs one
...
| `--keyword` / `-k`     | â€”                       | Comma-separated keywords (required unless `--mode catalog`)    |
...
| `--catalog`            | off                     | Sugar â€” also scrape the own catalog (folds into `--mode`)      |
| `--sample`             | 50                      | Locations sampled per city (evenly spread)                     |
| `--full`               | off                     | Census â€” every catalog location (ignores `--sample`)           |
...
| `--label`              | â€”                       | Human label stored on the run                                  |
| `--tenant` / `-t`      | â€”                       | Optional attribution to a client (does **not** scope storage)  |
| `--output` / `-o`      | `out/<brand>_<ts>.xlsx` | Workbook path                                                  |

**Workbook** â€” rendered by the shared exports writer, so it looks and reads like
`cli export public`: Contents Â· How to read this Â· Run Overview Â· Search Term
Scorecard Â· Geography Â· Competitor Landscape Â· Price & Discount Â· Availability Â·
Own Catalogue (catalog mode) Â· then Captured â€” Searches / Products / Own
Catalogue / Locations.
...
- Ephemeral: nothing in `search_snapshots` / `search_listings` / `sku_snapshots` â€” only the
...
- **Budget scheduler** â€” sets a campaign's daily budget from time/day rules (elevated during a window, back to a default otherwise).
- **Bid optimizer** â€” a ~15-min control loop that nudges a keyword's CPM to hold a target search position.

**Rules are the source of truth.** You create rules (`cm rules â€¦`); a **reconciler** compiles them into `job_schedules` rows; the runner fires the engines on schedule. See [campaign-manager.md](campaign-manager.md) for the design.

**Two ways to run every engine** (same as `cli scrape â€¦` vs `jobs run scrape.â€¦`):
...
> âš ï¸ **Dry-run is the default; nothing touches Blinkit unless the tenant is armed (`cm arm`) AND the run carries `--live`.** The engines read real budgets/positions in dry-run but write nothing. Going live is a deliberate per-tenant switch â€” see "Go live â€” arm the tenant" below. Engine reads open a browser + use the tenant's session, so run them where the VM would â€” locally is fine for dry-run testing. Never run the _plain_ `cli runner start` locally (it claims the VM's jobs and scrapes from your home IP); to drive the full queue â†’ schedule â†’ engine loop on a laptop, use **`cli runner start --only-cm`**, which serves only the campaign-manager lanes and fires only `cm.*` schedules (see the "Local Campaign-Manager testing" section of [jobs-runbook.md](jobs-runbook.md)).
...
| Shape                      | How                                                                            | Example                                     |
| -------------------------- | ------------------------------------------------------------------------------ | ------------------------------------------- |
| **Recurring daily window** | `--start-time`/`--end-time` (+ optional `--days`, `--start-date`/`--end-date`) | boost 4pmâ€“11pm every Fri/Sat/Sun this month |
| **Continuous (all day)**   | omit the times, set a date range                                               | around-the-clock for a 2-day sale           |
| **One-time span**          | `--once --date` (+ times)                                                      | a single night, 6pmâ€“2am                     |
| **Overnight**              | set end â‰¤ start time (e.g. `16:00`â†’`02:00`)                                    | window that crosses midnight                |

**Overnight tails belong to the start day**: a Sun 16:00â€“02:00 rule runs to **Mon 02:00**, and `--days friday,saturday,sunday` still covers Sunday's tail. `--days` is the weekday filter (empty = every day); it applies to budget and bid.

### Manage rules â€” `cm rules â€¦`
...
| `--default-budget`            | â‚¹ applied when no rule matches (schedule)                                      |
| `--name` / `--campaign-name`  | labels                                                                         |
| `--budget`                    | â‚¹ the rule applies (rule; on `add-budget-schedule` it creates one inline rule) |
| `--start-time` / `--end-time` | daily window `HH:MM` IST (end â‰¤ start = overnight)                             |
...
**Bid** (`add-bid`) â€” all of the above timing flags (with `--stop-time`/`--stop-date` instead of `--end-*`), plus:

| Flag                      | Notes                                                                                                     |
| ------------------------- | --------------------------------------------------------------------------------------------------------- |
| `--keyword`               | search keyword to chase                                                                                   |
| `--target`                | target sponsored position (e.g. `3`)                                                                      |
| `--min-bid` / `--max-bid` | CPM floor / ceiling (â‚¹)                                                                                   |
...
> One bid rule per (campaign, keyword) â€” the bid is campaign-wide; `--lat/--lon` only chooses where you measure.

### Reconcile â€” rules â†’ schedules
...
Here `--live` means "actually write **`job_schedules`**" (our own table) â€” reconcile **never** touches Blinkit. It's idempotent: re-running with unchanged rules is a no-op. (The V4 API will enqueue this for you on every edit.)

### Run the engines (dry-run testing)

```bash
python -m cli cm budget-scheduler -t <id>          # reads real budget â†’ "would set â‚¹X" â†’ writes nothing
python -m cli cm bid-optimizer    -t <id>          # reads live position â†’ "would bid â‚¹Y" â†’ writes nothing
python -m cli cm bid-optimizer    -t <id> --reset  # end-of-window: de-escalate closed keywords â†’ min_bid (no scrape)
```

Add `--live` to actually write to Blinkit (only takes effect once the tenant is **armed** â€” see below). History lands in `cm_run_log` (only real changes â€” no-op/hold rows go to Cloud Logging); bid runtime (last position/CPM) in `cm_bid_runtime`. `cm sync-campaign-data` is a stub.

**`--reset`** is the end-of-window mode: it sets each just-closed keyword's bid back to its `min_bid` (no position scrape), so a bid the optimizer pushed up doesn't keep spending high overnight. The reconciler fires this automatically at each window's stop time â€” you rarely run it by hand.

### Go live â€” arm the tenant (the cutover)

The whole automated loop is **dry by default**. Arming is a per-tenant switch (`cm_platform_accounts.live_armed`) that makes the reconciler stamp `--live` onto the engine schedules and the API's set-budget/reset pass live â€” so scheduled runs and UI actions write to Blinkit for real.

```bash
python -m cli cm set-advertiser -t <id> --id 19802   # one-time: the Blinkit ad-account id (required to arm)
python -m cli cm arm            -t <id>              # âš¡ ARM: set live_armed + reconcile â†’ schedules carry --live
python -m cli cm advertiser     -t <id>              # verify: "LIVE writes: âš¡ ARMED"
...
### Worked example â€” a solo dry-run pass
...
python -m cli cm reconcile         -t <id> --live      # rules â†’ job_schedules
...
Outside the rule engine â€” act on a single campaign right now. All are dry-run
...
(full command reference, troubleshooting) â€” they are not repeated here.

---

## Export â€” reports & raw data (`export`)
...
Two artifacts, deliberately separate â€” the report is a readable client deliverable,
...
### The client report â€” `export public`
...
| Option              | Meaning                                                                                                                                                                                                                 |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `-t, --tenant`      | Client id (**required**) â€” `cli tenant list`                                                                                                                                                                            |
| `--from` / `--to`   | Inclusive dates. Omit both and it uses **the last 7 days that have data**, not the last 7 days from today â€” public scrapes are weekly, so a today-anchored window often lands after the last scrape and returns nothing |
| `-c, --city`        | One city at a time (multi-city sections are not built)                                                                                                                                                                  |
| `--kind`            | `main` (default) Â· `combo` Â· `all` â€” combos are stocked selectively, so they are reported apart                                                                                                                         |
...
python -m cli export sample       # render a fixture workbook â€” no database needed
```
...
```bash
python -m cli export raw -t <uuid> --dry-run          # count first â€” always
...
| `--limit`                          | Cap rows per table â€” for a quick sample                                                     |
...
Row counts always print before anything is written â€” the volume is the thing worth
...
and exits non-zero if anything is wrong. Both run as scheduled jobs on the VM â€”
...
- Store credentials (`auth credentials set`) and run `auth login` **once** per tenant per
  platform. After that scrapers self-heal via `ensure()` â€” you do not run `auth` before
  each `scrape`
...
- `cli sync` and `scrape public --save` auto-create brand + marketplace rows (`ensure_refs`) â€” no manual seeding
- Public scraping is config-driven: `cli sync --file config.xlsx` before `cli scrape public-run` / `public-skus`
- The two public scrapes are independent commands with independent `scrape_job`s â€” run them on separate cadences (e.g. `public-skus` daily, `public-run` weekly)
