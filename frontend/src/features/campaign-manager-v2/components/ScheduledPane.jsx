...
const ConfirmDelete = ({ onConfirm }) => {
	...
	if (!armed) return <Action tone="danger" onClick={() => setArmed(true)}>Delete</Action>;
	return (
		<span className="inline-flex items-center gap-2">
			<button type="button" onClick={onConfirm} className="text-xs font-semibold text-danger hover:underline">
				Confirm delete
			</button>
...
const Chevron = ({ open }) => (
	<span className={`text-content-subtle transition-transform ${open ? "rotate-90" : ""}`}>›</span>
);
	...
	<li className="rounded-lg border border-border bg-surface">
		...
		>
			...
			<span className="min-w-0 flex-1">
				<span className="block truncate text-sm font-semibold text-content">{title}</span>
				<span className="block truncate text-xs text-content-muted">{subtitle}</span>
			</span>
			{chip && <span className="shrink-0 text-xs font-medium text-content">{chip}</span>}
			<Chevron open={open} />
		</button>
		{open && <div className="border-t border-border/70 px-3.5 py-3">{children}</div>}
	</li>
);
...
const BudgetItem = ({ schedule, onReset, resetJob, onDelete, onDeleteRule }) => {
	...
	return (
		<Row
			...
			chip={`${formatCurrency(schedule.default_budget)} default`}
			subtitle={`${schedule.campaign_name} · ${summary}`}
			open={open}
				...
				<>
					<ul className="space-y-1.5">
						{schedule.rules.length === 0 && (
							<li className="text-xs text-content-subtle">No windows — always the everyday budget.</li>
						)}
						{schedule.rules.map((r) =>
							mode === r.id ? (
								<li key={r.id}>
									<AddBudgetRuleForm scheduleId={schedule.id} editing={r} onDone={done} />
								</li>
							) : (
								<li
									key={r.id}
								...
								>
									...
									<span className="flex shrink-0 items-center gap-2.5">
										<Action tone="primary" onClick={() => setMode(r.id)}>edit</Action>
										<Action tone="danger" onClick={() => onDeleteRule(r.id)}>remove</Action>
									</span>
								</li>
							),
						)}
					...
					{mode === "add" ? (
						<div className="mt-2">
							<AddBudgetRuleForm scheduleId={schedule.id} onDone={done} />
						</div>
					) : (
						<div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1.5 border-t border-border/70 pt-2.5">
							<Action tone="primary" onClick={() => setMode("add")}>+ Window</Action>
							<Action onClick={() => setMode("schedule")}>Edit</Action>
...
const BidItem = ({ rule, onAction, onDelete }) => {
	...
	return (
		...
		>
			{editing ? (
				<AutomateBidForm editing={rule} onDone={() => setEditing(false)} />
			) : (
				<>
					<dl className="grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
						<dt className="text-content-subtle">Bid range</dt>
						<dd className="text-content">₹{rule.min_bid}–{rule.max_bid}</dd>
						<dt className="text-content-subtle">Measured at</dt>
						<dd className="truncate text-content">{rule.location_name || "—"}</dd>
						<dt className="text-content-subtle">When</dt>
						<dd className="truncate text-content">{describeTiming(rule)}</dd>
					</dl>
					<div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1.5 border-t border-border/70 pt-2.5">
						<Action tone="primary" onClick={() => setEditing(true)}>Edit</Action>
						{rule.state === "paused" && <Action onClick={() => onAction(rule.id, "resume")}>Resume</Action>}
						{rule.state === "active" && <Action onClick={() => onAction(rule.id, "pause")}>Pause</Action>}
						{rule.state !== "stopped" && <Action onClick={() => onAction(rule.id, "stop")}>Stop</Action>}
...
export const ScheduledPane = () => {
	...
	const doReset = (scheduleId) =>
		reset.mutate(scheduleId, { onSuccess: (data) => setResetJob({ scheduleId, jobId: data.job_id }) });
