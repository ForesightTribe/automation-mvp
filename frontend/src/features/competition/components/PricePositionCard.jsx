 ...
 * A comparison table (no chart): the numbers are the point.
 */
export const PricePositionCard = () => {
	...
	return (
		<Card
			...
			actions={<ViewToggle options={KIND_OPTIONS} value={kind} onChange={setKind} />}
		>
			...
			{!isLoading &&
				...
				(rows.length === 0 ? (
					...
					<DataTable columns={columns} rows={rows} rowKey={(r) => r.keyword} />
				))}
		</Card>
	);
};
