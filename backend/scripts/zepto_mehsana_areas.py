"""
Scrape Zepto dark stores for all Mehsana pincodes.
...
Run from backend/:
    python -m scripts.zepto_mehsana_areas
Output: zepto_mehsana_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import MEHSANA_PINCODES
...
# Mehsana bounding box — covers Mehsana + Patan + Chanasma + Kadi + Visnagar + Sidhpur + Unjha
_LAT_MIN, _LAT_MAX = 22.5, 24.5
_LNG_MIN, _LNG_MAX = 71.5, 73.5
...
async def main():
    ...
    print(f"Zepto Mehsana Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(MEHSANA_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(MEHSANA_PINCODES)
        for i, (pincode, label) in enumerate(MEHSANA_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Mehsana ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_mehsana_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Mehsana — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Mehsana — Store Count by City", generated)
