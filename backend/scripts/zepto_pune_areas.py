"""
Scrape Zepto dark stores for all Pune pincodes.
...
Run from backend/:
    python -m scripts.zepto_pune_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import PUNE_PINCODES
...
# Pune bounding box — rejects coordinates outside Pune region
_LAT_MIN, _LAT_MAX = 17.5, 19.2
_LNG_MIN, _LNG_MAX = 73.2, 75.5
...
async def main():
    ...
    print(f"Zepto Pune Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(PUNE_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(PUNE_PINCODES)
        for i, (pincode, label) in enumerate(PUNE_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Pune ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_pune_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Pune — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Pune — Store Count by City", generated)
