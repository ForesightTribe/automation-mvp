"""
Scrape Zepto dark stores for all Chhatrapati Sambhaji Nagar pincodes.
...
Run from backend/:
    python -m scripts.zepto_csn_areas
Output: zepto_csn_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import CSN_PINCODES
...
# CSN bounding box — covers Chhatrapati Sambhaji Nagar + Paithan + Gangapur + Vaijapur + Sillod + Kannad
_LAT_MIN, _LAT_MAX = 19.5, 21.0
_LNG_MIN, _LNG_MAX = 74.5, 76.5
...
async def main():
    ...
    print(f"Zepto Chhatrapati Sambhaji Nagar Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(CSN_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(CSN_PINCODES)
        for i, (pincode, label) in enumerate(CSN_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside CSN ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_csn_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Chhatrapati Sambhaji Nagar — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto CSN — Store Count by City", generated)
