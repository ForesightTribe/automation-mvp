"""
Scrape Zepto dark stores for all SAS Nagar (Mohali) pincodes.
...
Run from backend/:
    python -m scripts.zepto_mohali_areas
Output: zepto_mohali_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import MOHALI_PINCODES
...
# Mohali bounding box — covers SAS Nagar + Zirakpur + Kharar + Dera Bassi + New Chandigarh
_LAT_MIN, _LAT_MAX = 30.6, 31.0
_LNG_MIN, _LNG_MAX = 76.6, 77.0
...
async def main():
    ...
    print(f"Zepto SAS Nagar (Mohali) Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(MOHALI_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(MOHALI_PINCODES)
        for i, (pincode, label) in enumerate(MOHALI_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Mohali ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_mohali_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores SAS Nagar (Mohali) — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Mohali — Store Count by City", generated)
