"""
Scrape Zepto dark stores for all Hyderabad pincodes.
...
Run from backend/:
    python -m scripts.zepto_hyderabad_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import HYDERABAD_PINCODES
...
# Hyderabad bounding box — rejects coordinates outside Hyderabad region
_LAT_MIN, _LAT_MAX = 16.9, 18.3
_LNG_MIN, _LNG_MAX = 77.8, 79.2
...
async def main():
    ...
    print(f"Zepto Hyderabad Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(HYDERABAD_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        ctx = await browser.new_context(
            user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/125.0.0.0 Safari/537.36",
        )
        ...
        total = len(HYDERABAD_PINCODES)
        for i, (pincode, label) in enumerate(HYDERABAD_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Hyderabad ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_hyderabad_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Hyderabad — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Hyderabad — Store Count by City", generated)
