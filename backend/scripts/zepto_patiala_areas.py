"""
Scrape Zepto dark stores for all Patiala pincodes.
...
Run from backend/:
    python -m scripts.zepto_patiala_areas
Output: zepto_patiala_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import PATIALA_PINCODES
...
# Patiala bounding box — covers Patiala city + Rajpura + Nabha + Samana + Sangrur border
_LAT_MIN, _LAT_MAX = 29.8, 31.0
_LNG_MIN, _LNG_MAX = 76.0, 77.0
...
async def main():
    ...
    print(f"Zepto Patiala Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(PATIALA_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(PATIALA_PINCODES)
        for i, (pincode, label) in enumerate(PATIALA_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Patiala ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_patiala_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Patiala — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Patiala — Store Count by City", generated)
