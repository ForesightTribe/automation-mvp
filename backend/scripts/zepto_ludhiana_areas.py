"""
Scrape Zepto dark stores for all Ludhiana pincodes.
...
Run from backend/:
    python -m scripts.zepto_ludhiana_areas
Output: zepto_ludhiana_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import LUDHIANA_PINCODES
...
# Ludhiana bounding box — covers Ludhiana city + Khanna + Jagraon + Samrala
_LAT_MIN, _LAT_MAX = 30.4, 31.2
_LNG_MIN, _LNG_MAX = 75.4, 76.2
...
async def main():
    ...
    print(f"Zepto Ludhiana Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(LUDHIANA_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(LUDHIANA_PINCODES)
        for i, (pincode, label) in enumerate(LUDHIANA_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Ludhiana ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_ludhiana_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Ludhiana — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Ludhiana — Store Count by City", generated)
