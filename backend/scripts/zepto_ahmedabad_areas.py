"""
Scrape Zepto dark stores for all Ahmedabad pincodes.
...
Run from backend/:
    python -m scripts.zepto_ahmedabad_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import AHMEDABAD_PINCODES
...
# Ahmedabad bounding box — rejects coordinates outside Ahmedabad/Gujarat region
_LAT_MIN, _LAT_MAX = 22.6, 24.0
_LNG_MIN, _LNG_MAX = 71.8, 73.4
...
async def main():
    ...
    print(f"Zepto Ahmedabad Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(AHMEDABAD_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(AHMEDABAD_PINCODES)
        for i, (pincode, label) in enumerate(AHMEDABAD_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Ahmedabad ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_ahmedabad_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Ahmedabad — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Ahmedabad — Store Count by City", generated)
