"""
Scrape Zepto dark stores for all Hubballi pincodes.
...
Run from backend/:
    python -m scripts.zepto_hubballi_areas
Output: zepto_hubballi_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import HUBBALLI_PINCODES
...
# Hubballi bounding box — covers Hubballi city + Dharwad border areas
_LAT_MIN, _LAT_MAX = 15.2, 15.6
_LNG_MIN, _LNG_MAX = 74.9, 75.3
...
async def main():
    ...
    print(f"Zepto Hubballi Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(HUBBALLI_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(HUBBALLI_PINCODES)
        for i, (pincode, label) in enumerate(HUBBALLI_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Hubballi ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_hubballi_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Hubballi — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Hubballi — Store Count by City", generated)
