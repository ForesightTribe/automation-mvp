"""
Scrape Zepto dark stores for all Nashik pincodes.
...
Run from backend/:
    python -m scripts.zepto_nashik_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import NASHIK_PINCODES
...
_LAT_MIN, _LAT_MAX = 19.3, 21.0
_LNG_MIN, _LNG_MAX = 73.2, 75.0
...
async def main():
    ...
    print(f"Zepto Nashik Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(NASHIK_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(NASHIK_PINCODES)
        for i, (pincode, label) in enumerate(NASHIK_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Nashik ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_nashik_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Nashik — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Nashik — Store Count by City", generated)
