"""
Scrape Zepto dark stores for all Surat pincodes.
...
Run from backend/:
    python -m scripts.zepto_surat_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import SURAT_PINCODES
...
_LAT_MIN, _LAT_MAX = 19.8, 22.5
_LNG_MIN, _LNG_MAX = 72.4, 74.5
...
async def main():
    ...
    print(f"Zepto Surat Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(SURAT_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(SURAT_PINCODES)
        for i, (pincode, label) in enumerate(SURAT_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Surat ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_surat_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Surat — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Surat — Store Count by City", generated)
