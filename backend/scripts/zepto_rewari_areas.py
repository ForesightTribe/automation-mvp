"""
Scrape Zepto dark stores for all Rewari pincodes.
...
Run from backend/:
    python -m scripts.zepto_rewari_areas
Output: zepto_rewari_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import REWARI_PINCODES
...
# Rewari bounding box — covers Rewari + Dharuhera + Bawal + Mahendragarh + Narnaul + Kosli
_LAT_MIN, _LAT_MAX = 27.5, 28.5
_LNG_MIN, _LNG_MAX = 75.8, 76.8
...
async def main():
    ...
    print(f"Zepto Rewari Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(REWARI_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(REWARI_PINCODES)
        for i, (pincode, label) in enumerate(REWARI_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Rewari ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_rewari_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Rewari — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Rewari — Store Count by City", generated)
