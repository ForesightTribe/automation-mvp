"""
Scrape Zepto dark stores for all Chandigarh pincodes.
...
Run from backend/:
    python -m scripts.zepto_chandigarh_areas
Output: zepto_chandigarh_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import CHANDIGARH_PINCODES
...
# Chandigarh bounding box — covers Chandigarh UT all sectors
_LAT_MIN, _LAT_MAX = 30.6, 30.9
_LNG_MIN, _LNG_MAX = 76.6, 76.9
...
async def main():
    ...
    print(f"Zepto Chandigarh Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(CHANDIGARH_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(CHANDIGARH_PINCODES)
        for i, (pincode, label) in enumerate(CHANDIGARH_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Chandigarh ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_chandigarh_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Chandigarh — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Chandigarh — Store Count by City", generated)
