"""
Scrape Zepto dark stores for all Davangere pincodes.
...
Run from backend/:
    python -m scripts.zepto_davangere_areas
Output: zepto_davangere_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import DAVANGERE_PINCODES
...
# Davangere bounding box — covers Davangere + Harihara + Channagiri + Chitradurga + Shimoga border
_LAT_MIN, _LAT_MAX = 13.0, 15.0
_LNG_MIN, _LNG_MAX = 75.5, 76.8
...
async def main():
    ...
    print(f"Zepto Davangere Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(DAVANGERE_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(DAVANGERE_PINCODES)
        for i, (pincode, label) in enumerate(DAVANGERE_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Davangere ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_davangere_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Davangere — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Davangere — Store Count by City", generated)
