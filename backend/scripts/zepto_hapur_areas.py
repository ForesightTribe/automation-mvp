"""
Scrape Zepto dark stores for all Hapur pincodes.
...
Run from backend/:
    python -m scripts.zepto_hapur_areas
Output: zepto_hapur_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import HAPUR_PINCODES
...
# Hapur bounding box — covers Hapur + Pilkhuwa + Garhmukteshwar + Bulandshahr border
_LAT_MIN, _LAT_MAX = 28.4, 29.0
_LNG_MIN, _LNG_MAX = 77.6, 78.2
...
async def main():
    ...
    print(f"Zepto Hapur Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(HAPUR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(HAPUR_PINCODES)
        for i, (pincode, label) in enumerate(HAPUR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Hapur ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_hapur_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Hapur — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Hapur — Store Count by City", generated)
