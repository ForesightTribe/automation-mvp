"""
Scrape Zepto dark stores for all Kochi pincodes.
...
Run from backend/:
    python -m scripts.zepto_kochi_areas
Output: zepto_kochi_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import KOCHI_PINCODES
...
# Kochi bounding box — covers Fort Kochi, Ernakulam, Aluva, Kakkanad, Edappally
_LAT_MIN, _LAT_MAX = 9.7, 10.5
_LNG_MIN, _LNG_MAX = 76.0, 76.8
...
async def main():
    ...
    print(f"Zepto Kochi Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(KOCHI_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(KOCHI_PINCODES)
        for i, (pincode, label) in enumerate(KOCHI_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Kochi ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_kochi_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Kochi — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Kochi — Store Count by City", generated)
