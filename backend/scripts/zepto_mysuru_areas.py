"""
Scrape Zepto dark stores for all Mysuru pincodes.
...
Run from backend/:
    python -m scripts.zepto_mysuru_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import MYSURU_PINCODES
...
_LAT_MIN, _LAT_MAX = 11.7, 13.0
_LNG_MIN, _LNG_MAX = 75.9, 77.5
...
async def main():
    ...
    print(f"Zepto Mysuru Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(MYSURU_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(MYSURU_PINCODES)
        for i, (pincode, label) in enumerate(MYSURU_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Mysuru ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_mysuru_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Mysuru — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Mysuru — Store Count by City", generated)
