"""
Scrape Zepto dark stores for all Ambala pincodes.
...
Run from backend/:
    python -m scripts.zepto_ambala_areas
Output: zepto_ambala_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import AMBALA_PINCODES
...
# Ambala bounding box — covers Ambala + Yamunanagar + Panchkula + Naraingarh + Kurukshetra border
_LAT_MIN, _LAT_MAX = 29.8, 31.0
_LNG_MIN, _LNG_MAX = 76.4, 77.4
...
async def main():
    ...
    print(f"Zepto Ambala Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(AMBALA_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(AMBALA_PINCODES)
        for i, (pincode, label) in enumerate(AMBALA_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Ambala ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_ambala_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Ambala — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Ambala — Store Count by City", generated)
