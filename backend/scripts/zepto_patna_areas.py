"""
Scrape Zepto dark stores for all Patna pincodes.
...
Run from backend/:
    python -m scripts.zepto_Patna_areas
Output: zepto_Patna_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import PATNA_PINCODES
...
# Patna bounding box — covers Patna city + Samalkha + Israna + Madlauda + Bapoli + Sanauli
_LAT_MIN, _LAT_MAX = 25.3, 25.9
_LNG_MIN, _LNG_MAX = 84.8, 85.5
...
async def main():
    ...
    print(f"Zepto Patna Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(PATNA_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(PATNA_PINCODES)
        for i, (pincode, label) in enumerate(PATNA_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Patna ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_Patna_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Patna — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Patna — Store Count by City", generated)
