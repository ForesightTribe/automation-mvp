"""
Scrape Zepto dark stores for all Vellore pincodes.
...
Run from backend/:
    python -m scripts.zepto_vellore_areas
Output: zepto_vellore_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import VELLORE_PINCODES
...
# Vellore bounding box — covers Vellore + Ranipet + Tirupattur + Krishnagiri + Dharmapuri border
...
async def main():
    ...
    print(f"Zepto Vellore Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(VELLORE_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(VELLORE_PINCODES)
        for i, (pincode, label) in enumerate(VELLORE_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Vellore ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_vellore_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Vellore — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Vellore — Store Count by City", generated)
