"""
Scrape Zepto dark stores for all Gorakhpur pincodes.
...
Run from backend/:
    python -m scripts.zepto_gorakhpur_areas
Output: zepto_gorakhpur_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import GORAKHPUR_PINCODES
...
# Gorakhpur bounding box — covers Gorakhpur city + Sahjanwa + Campierganj + Chauri Chaura
_LAT_MIN, _LAT_MAX = 26.5, 27.5
_LNG_MIN, _LNG_MAX = 83.0, 84.0
...
async def main():
    ...
    print(f"Zepto Gorakhpur Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(GORAKHPUR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(GORAKHPUR_PINCODES)
        for i, (pincode, label) in enumerate(GORAKHPUR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Gorakhpur ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_gorakhpur_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Gorakhpur — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Gorakhpur — Store Count by City", generated)
