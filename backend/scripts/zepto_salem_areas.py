"""
Scrape Zepto dark stores for all Salem pincodes.
...
Run from backend/:
    python -m scripts.zepto_salem_areas
Output: zepto_salem_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import SALEM_PINCODES
...
# Salem bounding box — covers Salem city + Samalkha + Israna + Madlauda + Bapoli + Sanauli
_LAT_MIN, _LAT_MAX = 11.4, 12.0
_LNG_MIN, _LNG_MAX = 78.0, 78.6
...
async def main():
    ...
    print(f"Zepto Salem Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(SALEM_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(SALEM_PINCODES)
        for i, (pincode, label) in enumerate(SALEM_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Salem ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_salem_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Salem — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Salem — Store Count by City", generated)
