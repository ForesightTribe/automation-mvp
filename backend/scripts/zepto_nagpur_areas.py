"""
Scrape Zepto dark stores for all Nagpur pincodes.
...
Run from backend/:
    python -m scripts.zepto_nagpur_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import NAGPUR_PINCODES
...
# Nagpur bounding box — covers Nagpur city + Nagpur district + Wardha
_LAT_MIN, _LAT_MAX = 20.5, 21.7
_LNG_MIN, _LNG_MAX = 78.2, 80.0
...
async def main():
    ...
    print(f"Zepto Nagpur Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(NAGPUR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(NAGPUR_PINCODES)
        for i, (pincode, label) in enumerate(NAGPUR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Nagpur ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_nagpur_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Nagpur — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Nagpur — Store Count by City", generated)
