"""
Scrape Zepto dark stores for all Visakhapatnam pincodes.
...
Run from backend/:
    python -m scripts.zepto_Visakhapatnam_areas
Output: zepto_Visakhapatnam_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import VISAKHAPATNAM_PINCODES
...
# Visakhapatnam bounding box — covers Visakhapatnam city + Samalkha + Israna + Madlauda + Bapoli + Sanauli
_LAT_MIN, _LAT_MAX = 17.0, 18.5
_LNG_MIN, _LNG_MAX = 82.5, 83.5
...
async def main():
    ...
    print(f"Zepto Visakhapatnam Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(VISAKHAPATNAM_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(VISAKHAPATNAM_PINCODES)
        for i, (pincode, label) in enumerate(VISAKHAPATNAM_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Visakhapatnam ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_Visakhapatnam_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Visakhapatnam — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Visakhapatnam — Store Count by City", generated)
