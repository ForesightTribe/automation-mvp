"""
Scrape Zepto dark stores for all Kota pincodes.
...
Run from backend/:
    python -m scripts.zepto_kota_areas
Output: zepto_kota_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import KOTA_PINCODES
...
# Kota bounding box — covers Kota city + Baran + Sangod + Ramganj Mandi
_LAT_MIN, _LAT_MAX = 24.5, 25.5
...
async def main():
    ...
    print(f"Zepto Kota Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(KOTA_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(KOTA_PINCODES)
        for i, (pincode, label) in enumerate(KOTA_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Kota ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_kota_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Kota — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Kota — Store Count by City", generated)
