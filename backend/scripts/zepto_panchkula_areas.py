"""
Scrape Zepto dark stores for all Panchkula pincodes.
...
Run from backend/:
    python -m scripts.zepto_panchkula_areas
Output: zepto_panchkula_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import PANCHKULA_PINCODES
...
# Panchkula bounding box — covers Panchkula sectors + Kalka + Pinjore + Morni Hills
_LAT_MIN, _LAT_MAX = 30.5, 31.2
_LNG_MIN, _LNG_MAX = 76.6, 77.2
...
async def main():
    ...
    print(f"Zepto Panchkula Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(PANCHKULA_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(PANCHKULA_PINCODES)
        for i, (pincode, label) in enumerate(PANCHKULA_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Panchkula ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_panchkula_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Panchkula — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Panchkula — Store Count by City", generated)
