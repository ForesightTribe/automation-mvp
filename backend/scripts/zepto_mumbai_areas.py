"""
Scrape Zepto dark stores for all Mumbai pincodes.
...
Run from backend/:
    python -m scripts.zepto_mumbai_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import MUMBAI_PINCODES
...
# Mumbai bounding box — rejects coordinates outside Mumbai/MMR region
_LAT_MIN, _LAT_MAX = 18.5, 19.6
_LNG_MIN, _LNG_MAX = 72.6, 73.4
...
async def main():
    ...
    print(f"Zepto Mumbai Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(MUMBAI_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(MUMBAI_PINCODES)
        for i, (pincode, label) in enumerate(MUMBAI_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Mumbai ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_mumbai_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Mumbai — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Mumbai — Store Count by City", generated)
