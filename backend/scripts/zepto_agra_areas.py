"""
Scrape Zepto dark stores for all Agra pincodes.
...
Run from backend/:
    python -m scripts.zepto_agra_areas
Output: zepto_agra_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import AGRA_PINCODES
...
# Agra bounding box — covers Agra city + Sikandra + Dayalbagh + Fatehabad + Etmadpur
_LAT_MIN, _LAT_MAX = 26.9, 27.5
_LNG_MIN, _LNG_MAX = 77.8, 78.4
...
async def main():
    ...
    print(f"Zepto Agra Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(AGRA_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(AGRA_PINCODES)
        for i, (pincode, label) in enumerate(AGRA_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Agra ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_agra_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Agra — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Agra — Store Count by City", generated)
