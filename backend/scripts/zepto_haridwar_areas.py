"""
Scrape Zepto dark stores for all Haridwar pincodes.
...
Run from backend/:
    python -m scripts.zepto_haridwar_areas
Output: zepto_haridwar_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import HARIDWAR_PINCODES
...
# Haridwar bounding box — covers Haridwar + Roorkee + Laksar + Bahadrabad + Kotdwar border
_LAT_MIN, _LAT_MAX = 29.6, 30.3
_LNG_MIN, _LNG_MAX = 77.7, 78.4
...
async def main():
    ...
    print(f"Zepto Haridwar Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(HARIDWAR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(HARIDWAR_PINCODES)
        for i, (pincode, label) in enumerate(HARIDWAR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Haridwar ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_haridwar_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Haridwar — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Haridwar — Store Count by City", generated)
