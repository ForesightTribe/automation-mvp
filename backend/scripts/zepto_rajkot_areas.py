"""
Scrape Zepto dark stores for all Rajkot pincodes.
...
Run from backend/:
    python -m scripts.zepto_rajkot_areas
Output: zepto_rajkot_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import RAJKOT_PINCODES
...
# Rajkot bounding box — covers Rajkot city + Morbi + Gondal + Jetpur + Wankaner + Dhoraji
_LAT_MIN, _LAT_MAX = 21.5, 23.0
_LNG_MIN, _LNG_MAX = 70.0, 71.5
...
async def main():
    ...
    print(f"Zepto Rajkot Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(RAJKOT_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(RAJKOT_PINCODES)
        for i, (pincode, label) in enumerate(RAJKOT_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Rajkot ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_rajkot_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Rajkot — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Rajkot — Store Count by City", generated)
