"""
Scrape Zepto dark stores for all Valsad pincodes.
...
Run from backend/:
    python -m scripts.zepto_valsad_areas
Output: zepto_valsad_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import VALSAD_PINCODES
...
# Valsad bounding box — covers Valsad + Pardi + Umargam + Navsari + Gandevi + Bilimora + Vapi
_LAT_MIN, _LAT_MAX = 20.0, 21.5
_LNG_MIN, _LNG_MAX = 72.5, 73.5
...
async def main():
    ...
    print(f"Zepto Valsad Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(VALSAD_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(VALSAD_PINCODES)
        for i, (pincode, label) in enumerate(VALSAD_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Valsad ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_valsad_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Valsad — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Valsad — Store Count by City", generated)
