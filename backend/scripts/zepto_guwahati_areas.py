"""
Scrape Zepto dark stores for all Guwahati pincodes.
...
Run from backend/:
    python -m scripts.zepto_guwahati_areas
Output: zepto_guwahati_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import GUWAHATI_PINCODES
...
# Guwahati bounding box — covers Guwahati city + Samalkha + Israna + Madlauda + Bapoli + Sanauli
_LAT_MIN, _LAT_MAX = 25.8, 26.3
_LNG_MIN, _LNG_MAX = 91.4, 92.0
...
async def main():
    ...
    print(f"Zepto Guwahati Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(GUWAHATI_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(GUWAHATI_PINCODES)
        for i, (pincode, label) in enumerate(GUWAHATI_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Guwahati ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_guwahati_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Guwahati — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Guwahati — Store Count by City", generated)
