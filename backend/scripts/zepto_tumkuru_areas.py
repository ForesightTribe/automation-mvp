"""
Scrape Zepto dark stores for all Tumkuru pincodes.
...
Run from backend/:
    python -m scripts.zepto_tumkuru_areas
Output: zepto_tumkuru_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import TUMKURU_PINCODES
...
# Tumkuru bounding box — covers Tumkuru + Tiptur + Gubbi + Kunigal + Madhugiri + Sira + Pavagada + Koratagere
_LAT_MIN, _LAT_MAX = 12.5, 14.5
_LNG_MIN, _LNG_MAX = 76.0, 77.5
...
async def main():
    ...
    print(f"Zepto Tumkuru Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(TUMKURU_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(TUMKURU_PINCODES)
        for i, (pincode, label) in enumerate(TUMKURU_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Tumkuru ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_tumkuru_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Tumkuru — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Tumkuru — Store Count by City", generated)
