"""
Scrape Zepto dark stores for all Varanasi pincodes.
...
Run from backend/:
    python -m scripts.zepto_varanasi_areas
Output: zepto_varanasi_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import VARANASI_PINCODES
...
# Varanasi bounding box — covers Varanasi + Chandauli + Mirzapur + Jaunpur + Bhadohi
_LAT_MIN, _LAT_MAX = 24.8, 26.2
_LNG_MIN, _LNG_MAX = 82.5, 84.0
...
async def main():
    ...
    print(f"Zepto Varanasi Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(VARANASI_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(VARANASI_PINCODES)
        for i, (pincode, label) in enumerate(VARANASI_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Varanasi ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_varanasi_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Varanasi — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Varanasi — Store Count by City", generated)
