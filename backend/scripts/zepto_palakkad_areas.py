"""
Scrape Zepto dark stores for all Palakkad pincodes.
...
Run from backend/:
    python -m scripts.zepto_palakkad_areas
Output: zepto_palakkad_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import PALAKKAD_PINCODES
...
# Palakkad bounding box — covers Palakkad city + Ottapalam + Shornur + Malappuram border
_LAT_MIN, _LAT_MAX = 10.3, 11.3
_LNG_MIN, _LNG_MAX = 76.3, 77.0
...
async def main():
    ...
    print(f"Zepto Palakkad Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(PALAKKAD_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(PALAKKAD_PINCODES)
        for i, (pincode, label) in enumerate(PALAKKAD_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Palakkad ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_palakkad_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Palakkad — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Palakkad — Store Count by City", generated)
