"""
Scrape Zepto dark stores for all Jalandhar pincodes.
...
Run from backend/:
    python -m scripts.zepto_jalandhar_areas
Output: zepto_jalandhar_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import JALANDHAR_PINCODES
...
# Jalandhar bounding box — covers Jalandhar + Kapurthala + Phagwara + Hoshiarpur + Pathankot + Gurdaspur
_LAT_MIN, _LAT_MAX = 30.8, 32.5
_LNG_MIN, _LNG_MAX = 74.8, 76.5
...
async def main():
    ...
    print(f"Zepto Jalandhar Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(JALANDHAR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(JALANDHAR_PINCODES)
        for i, (pincode, label) in enumerate(JALANDHAR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Jalandhar ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_jalandhar_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Jalandhar — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Jalandhar — Store Count by City", generated)
