"""
Scrape Zepto dark stores for all Thiruvananthapuram pincodes.
...
Run from backend/:
    python -m scripts.zepto_thiruvananthapuram_areas
Output: zepto_thiruvananthapuram_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import THIRUVANANTHAPURAM_PINCODES
...
# Thiruvananthapuram bounding box — covers Thiruvananthapuram city + Samalkha + Israna + Madlauda + Bapoli + Sanauli
_LAT_MIN, _LAT_MAX = 8.3, 9.0
_LNG_MIN, _LNG_MAX = 76.7, 77.4
...
async def main():
    ...
    print(f"Zepto Thiruvananthapuram Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(THIRUVANANTHAPURAM_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(THIRUVANANTHAPURAM_PINCODES)
        for i, (pincode, label) in enumerate(THIRUVANANTHAPURAM_PINCODES.items(), 1):
            ...
            preds = []
            for pred in preds_by_pin:
                pid = pred.get("place_id")
                if pid and pid not in seen_ids:
                    seen_ids.add(pid)
                    pred["_search_source"] = "pincode"
                    preds.append(pred)
            for pred in preds_by_name:
                pid = pred.get("place_id")
                if pid and pid not in seen_ids:
                    seen_ids.add(pid)
                    pred["_search_source"] = "area_name"
                    preds.append(pred)
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Thiruvananthapuram ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_thiruvananthapuram_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Thiruvananthapuram — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Thiruvananthapuram — Store Count by City", generated)
