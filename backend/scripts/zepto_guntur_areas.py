"""
Scrape Zepto dark stores for all Guntur pincodes.
...
Run from backend/:
    python -m scripts.zepto_guntur_areas
Output: zepto_guntur_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import GUNTUR_PINCODES
...
# Guntur bounding box — covers Guntur + Tenali + Narasaraopet + Bapatla + Ongole border
_LAT_MIN, _LAT_MAX = 15.0, 16.5
_LNG_MIN, _LNG_MAX = 79.0, 80.5
...
async def main():
    ...
    print(f"Zepto Guntur Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(GUNTUR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(GUNTUR_PINCODES)
        for i, (pincode, label) in enumerate(GUNTUR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Guntur ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_guntur_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Guntur — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Guntur — Store Count by City", generated)
