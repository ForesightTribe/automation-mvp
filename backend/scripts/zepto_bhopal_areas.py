"""
Scrape Zepto dark stores for all Bhopal pincodes.
...
Run from backend/:
    python -m scripts.zepto_Bhopal_areas
Output: zepto_Bhopal_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import BHOPAL_PINCODES
...
# Bhopal bounding box — covers Bhopal city + Samalkha + Israna + Madlauda + Bapoli + Sanauli
_LAT_MIN, _LAT_MAX = 22.8, 23.5
_LNG_MIN, _LNG_MAX = 77.0, 77.8
...
async def main():
    ...
    print(f"Zepto Bhopal Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(BHOPAL_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(BHOPAL_PINCODES)
        for i, (pincode, label) in enumerate(BHOPAL_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Bhopal ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_Bhopal_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Bhopal — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Bhopal — Store Count by City", generated)
