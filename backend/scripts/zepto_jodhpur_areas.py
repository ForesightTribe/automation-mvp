"""
Scrape Zepto dark stores for all Jodhpur pincodes.
...
Run from backend/:
    python -m scripts.zepto_Jodhpur_areas
Output: zepto_Jodhpur_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import JODHPUR_PINCODES
...
# Jodhpur bounding box — covers Jodhpur city + Samalkha + Israna + Madlauda + Bapoli + Sanauli
_LAT_MIN, _LAT_MAX = 25.8, 26.5
_LNG_MIN, _LNG_MAX = 72.8, 73.5
...
async def main():
    ...
    print(f"Zepto Jodhpur Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(JODHPUR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(JODHPUR_PINCODES)
        for i, (pincode, label) in enumerate(JODHPUR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Jodhpur ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_Jodhpur_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Jodhpur — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Jodhpur — Store Count by City", generated)
