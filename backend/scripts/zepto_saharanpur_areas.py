"""
Scrape Zepto dark stores for all Saharanpur pincodes.
...
Run from backend/:
    python -m scripts.zepto_saharanpur_areas
Output: zepto_saharanpur_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import SAHARANPUR_PINCODES
...
# Saharanpur bounding box — covers Saharanpur city + Behat + Gangoh + Deoband + Nakur + Rampur Maniharan
_LAT_MIN, _LAT_MAX = 29.5, 30.3
_LNG_MIN, _LNG_MAX = 77.0, 78.0
...
async def main():
    ...
    print(f"Zepto Saharanpur Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(SAHARANPUR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(SAHARANPUR_PINCODES)
        for i, (pincode, label) in enumerate(SAHARANPUR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Saharanpur ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_saharanpur_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Saharanpur — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Saharanpur — Store Count by City", generated)
