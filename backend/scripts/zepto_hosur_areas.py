"""
Scrape Zepto dark stores for all Hosur pincodes.
...
Run from backend/:
    python -m scripts.zepto_hosur_areas
Output: zepto_hosur_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import HOSUR_PINCODES
...
# Hosur bounding box — covers Hosur + Denkanikottai + Krishnagiri border (Tamil Nadu / Karnataka)
_LAT_MIN, _LAT_MAX = 12.0, 13.0
_LNG_MIN, _LNG_MAX = 77.5, 78.5
...
async def main():
    ...
    print(f"Zepto Hosur Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(HOSUR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(HOSUR_PINCODES)
        for i, (pincode, label) in enumerate(HOSUR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Hosur ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_hosur_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Hosur — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Hosur — Store Count by City", generated)
