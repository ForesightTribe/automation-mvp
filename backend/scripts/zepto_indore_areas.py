"""
Scrape Zepto dark stores for all Indore pincodes.
...
Run from backend/:
    python -m scripts.zepto_indore_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import INDORE_PINCODES
...
# Indore bounding box — covers Indore city + Dewas + Pithampur + Mhow
_LAT_MIN, _LAT_MAX = 22.3, 23.3
_LNG_MIN, _LNG_MAX = 75.3, 76.5
...
async def main():
    ...
    print(f"Zepto Indore Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(INDORE_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(INDORE_PINCODES)
        for i, (pincode, label) in enumerate(INDORE_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Indore ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_indore_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Indore — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Indore — Store Count by City", generated)
