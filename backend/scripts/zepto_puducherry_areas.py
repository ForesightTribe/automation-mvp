"""
Scrape Zepto dark stores for all Puducherry pincodes.
...
Run from backend/:
    python -m scripts.zepto_puducherry_areas
Output: zepto_puducherry_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import PUDUCHERRY_PINCODES
...
# Puducherry bounding box — covers Puducherry UT + Karaikal enclave
_LAT_MIN, _LAT_MAX = 10.5, 12.1
_LNG_MIN, _LNG_MAX = 79.5, 80.0
...
async def main():
    ...
    print(f"Zepto Puducherry Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(PUDUCHERRY_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(PUDUCHERRY_PINCODES)
        for i, (pincode, label) in enumerate(PUDUCHERRY_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Puducherry ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_puducherry_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Puducherry — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Puducherry — Store Count by City", generated)
