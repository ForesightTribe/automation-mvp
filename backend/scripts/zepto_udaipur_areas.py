"""
Scrape Zepto dark stores for all Udaipur pincodes.
...
Run from backend/:
    python -m scripts.zepto_udaipur_areas
Output: zepto_udaipur_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import UDAIPUR_PINCODES
...
# Udaipur bounding box — covers Udaipur + Rajsamand + Nathdwara + Sirohi + Pratapgarh
_LAT_MIN, _LAT_MAX = 23.5, 25.5
_LNG_MIN, _LNG_MAX = 72.8, 75.0
...
async def main():
    ...
    print(f"Zepto Udaipur Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(UDAIPUR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(UDAIPUR_PINCODES)
        for i, (pincode, label) in enumerate(UDAIPUR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Udaipur ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_udaipur_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Udaipur — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Udaipur — Store Count by City", generated)
