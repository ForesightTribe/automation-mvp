"""
Scrape Zepto dark stores for all Jaipur pincodes.
...
Run from backend/:
    python -m scripts.zepto_jaipur_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import JAIPUR_PINCODES
...
_LAT_MIN, _LAT_MAX = 24.5, 28.5
_LNG_MIN, _LNG_MAX = 73.5, 77.8
...
async def main():
    ...
    print(f"Zepto Jaipur Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(JAIPUR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(JAIPUR_PINCODES)
        for i, (pincode, label) in enumerate(JAIPUR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Jaipur ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_jaipur_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Jaipur — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Jaipur — Store Count by City", generated)
