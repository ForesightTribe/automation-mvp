"""
Scrape Zepto dark stores for all Panipat pincodes.
...
Run from backend/:
    python -m scripts.zepto_panipat_areas
Output: zepto_panipat_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import PANIPAT_PINCODES
...
# Panipat bounding box — covers Panipat city + Samalkha + Israna + Madlauda + Bapoli + Sanauli
_LAT_MIN, _LAT_MAX = 29.1, 29.7
_LNG_MIN, _LNG_MAX = 76.7, 77.2
...
async def main():
    ...
    print(f"Zepto Panipat Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(PANIPAT_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(PANIPAT_PINCODES)
        for i, (pincode, label) in enumerate(PANIPAT_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Panipat ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_panipat_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Panipat — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Panipat — Store Count by City", generated)
