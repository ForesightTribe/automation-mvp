"""
Scrape Zepto dark stores for all Karimnagar pincodes.
...
Run from backend/:
    python -m scripts.zepto_karimnagar_areas
Output: zepto_karimnagar_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import KARIMNAGAR_PINCODES
...
# Karimnagar bounding box — covers Karimnagar + Jagitial + Sircilla + Ramagundam + Peddapalli
_LAT_MIN, _LAT_MAX = 17.8, 19.0
_LNG_MIN, _LNG_MAX = 78.5, 80.0
...
async def main():
    ...
    print(f"Zepto Karimnagar Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(KARIMNAGAR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(KARIMNAGAR_PINCODES)
        for i, (pincode, label) in enumerate(KARIMNAGAR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Karimnagar ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_karimnagar_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Karimnagar — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Karimnagar — Store Count by City", generated)
