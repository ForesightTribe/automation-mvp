"""
Scrape Zepto dark stores for all Meerut pincodes.
...
Run from backend/:
    python -m scripts.zepto_meerut_areas
Output: zepto_meerut_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import MEERUT_PINCODES
...
# Meerut bounding box — covers Meerut city + Cantonment + Modipuram + Mawana
_LAT_MIN, _LAT_MAX = 28.8, 29.2
_LNG_MIN, _LNG_MAX = 77.5, 78.0
...
async def main():
    ...
    print(f"Zepto Meerut Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(MEERUT_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(MEERUT_PINCODES)
        for i, (pincode, label) in enumerate(MEERUT_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Meerut ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_meerut_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Meerut — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Meerut — Store Count by City", generated)
