"""
Scrape Zepto dark stores for all Bhiwadi pincodes.
...
Run from backend/:
    python -m scripts.zepto_bhiwadi_areas
Output: zepto_bhiwadi_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import BHIWADI_PINCODES
...
# Bhiwadi bounding box — covers Bhiwadi industrial area + Alwar border + Gurugram border
_LAT_MIN, _LAT_MAX = 28.0, 28.4
_LNG_MIN, _LNG_MAX = 76.7, 77.1
...
async def main():
    ...
    print(f"Zepto Bhiwadi Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(BHIWADI_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(BHIWADI_PINCODES)
        for i, (pincode, label) in enumerate(BHIWADI_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Bhiwadi ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_bhiwadi_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Bhiwadi — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Bhiwadi — Store Count by City", generated)
