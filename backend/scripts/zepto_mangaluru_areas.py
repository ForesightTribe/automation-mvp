"""
Scrape Zepto dark stores for all Mangaluru pincodes.
...
Run from backend/:
    python -m scripts.zepto_mangaluru_areas
Output: zepto_mangaluru_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import MANGALURU_PINCODES
...
# Mangaluru bounding box — covers Mangaluru city + Samalkha + Israna + Madlauda + Bapoli + Sanauli
_LAT_MIN, _LAT_MAX = 12.5, 13.2
_LNG_MIN, _LNG_MAX = 74.5, 75.2
...
async def main():
    ...
    print(f"Zepto Mangaluru Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(MANGALURU_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(MANGALURU_PINCODES)
        for i, (pincode, label) in enumerate(MANGALURU_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Mangaluru ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_mangaluru_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Mangaluru — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Mangaluru — Store Count by City", generated)
