"""
Scrape Zepto dark stores for all Chennai pincodes.
...
Run from backend/:
    python -m scripts.zepto_chennai_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import CHENNAI_PINCODES
...
# Chennai bounding box — rejects coordinates outside Chennai region
_LAT_MIN, _LAT_MAX = 12.5, 13.5
...
async def main():
    ...
    print(f"Zepto Chennai Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(CHENNAI_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(CHENNAI_PINCODES)
        for i, (pincode, label) in enumerate(CHENNAI_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Chennai ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_chennai_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Chennai — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Chennai — Store Count by City", generated)
