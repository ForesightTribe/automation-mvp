"""
Scrape Zepto dark stores for all Karnal pincodes.
...
Run from backend/:
    python -m scripts.zepto_karnal_areas
Output: zepto_karnal_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import KARNAL_PINCODES
...
# Karnal bounding box — covers Karnal + Nilokheri + Indri + Gharaunda + Assandh + Taraori
_LAT_MIN, _LAT_MAX = 29.4, 30.1
...
async def main():
    ...
    print(f"Zepto Karnal Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(KARNAL_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(KARNAL_PINCODES)
        for i, (pincode, label) in enumerate(KARNAL_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Karnal ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_karnal_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Karnal — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Karnal — Store Count by City", generated)
