"""
Scrape Zepto dark stores for all 180 NCR pincodes.
For each pincode, fetches ALL autocomplete suggestions, calls place/details for
each, and takes the lat/lng as-is (APPROXIMATE or not — coordinates are real).
No grid scan. No reverse geocoding.

Run from backend/:
    python -m scripts.zepto_targeted_areas
"""
import asyncio
import sys
...
async def get_predictions(query: str, ctx) -> list[dict]:
    """Return all autocomplete predictions for a query."""
...
async def get_details(place_id: str, ctx) -> dict | None:
    """Call place/details and return lat/lng + address metadata.
    Takes the coords as-is from geometry.location regardless of location_type."""
    for attempt in range(2):
        try:
            r = await ctx.request.get(
...
async def main():
    ...
    print(f"Zepto NCR Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(DELHI_NCR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        for i, (pincode, label) in enumerate(DELHI_NCR_PINCODES.items(), 1):
            ...
            # Search by pincode AND by area name — combine unique place_ids
            ...
            for pred in preds:
                ...
                if not (27.8 <= lat <= 29.2 and 76.5 <= lng <= 78.0):
                    print(f"  ✗ {name[:55]} → outside NCR ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_exact_stores_{ts}.xlsx"
    ...
    ws.append(hdr)
    for row in sorted(rows, key=lambda x: (x["city"], x["pincode"] or "999999")):
    ...
    apply_heading(ws, "Zepto Dark Stores NCR — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto NCR — Store Count by City", generated)
