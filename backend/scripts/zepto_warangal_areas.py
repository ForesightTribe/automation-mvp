"""
Scrape Zepto dark stores for all Warangal pincodes.
...
Run from backend/:
    python -m scripts.zepto_warangal_areas
Output: zepto_warangal_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import WARANGAL_PINCODES
...
# Warangal bounding box — covers Warangal + Hanamkonda + Kazipet + Parkal + Narsampet
_LAT_MIN, _LAT_MAX = 17.5, 18.5
_LNG_MIN, _LNG_MAX = 79.2, 80.2
...
async def main():
    ...
    print(f"Zepto Warangal Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(WARANGAL_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(WARANGAL_PINCODES)
        for i, (pincode, label) in enumerate(WARANGAL_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Warangal ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_warangal_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Warangal — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Warangal — Store Count by City", generated)
