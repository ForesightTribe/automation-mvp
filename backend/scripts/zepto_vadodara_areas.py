"""
Scrape Zepto dark stores for all Vadodara pincodes.
...
Run from backend/:
    python -m scripts.zepto_vadodara_areas
Output: zepto_vadodara_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import VADODARA_PINCODES
...
# Vadodara bounding box — covers Vadodara city + Makarpura + Harni + Manjalpur
_LAT_MIN, _LAT_MAX = 22.1, 22.6
_LNG_MIN, _LNG_MAX = 73.0, 73.6
...
async def main():
    ...
    print(f"Zepto Vadodara Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(VADODARA_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(VADODARA_PINCODES)
        for i, (pincode, label) in enumerate(VADODARA_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Vadodara ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_vadodara_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Vadodara — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Vadodara — Store Count by City", generated)
