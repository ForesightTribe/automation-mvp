"""
Scrape Zepto dark stores for all Coimbatore pincodes.
...
Run from backend/:
    python -m scripts.zepto_coimbatore_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import COIMBATORE_PINCODES
...
_LAT_MIN, _LAT_MAX = 10.2, 11.6
_LNG_MIN, _LNG_MAX = 76.5, 77.8
...
async def main():
    ...
    print(f"Zepto Coimbatore Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(COIMBATORE_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(COIMBATORE_PINCODES)
        for i, (pincode, label) in enumerate(COIMBATORE_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Coimbatore ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_coimbatore_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Coimbatore — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Coimbatore — Store Count by City", generated)
