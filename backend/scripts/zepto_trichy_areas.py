"""
Scrape Zepto dark stores for all Trichy pincodes.
...
Run from backend/:
    python -m scripts.zepto_trichy_areas
Output: zepto_trichy_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import TRICHY_PINCODES
...
# Trichy bounding box — covers Trichy city + Samalkha + Israna + Madlauda + Bapoli + Sanauli
_LAT_MIN, _LAT_MAX = 10.4, 11.0
_LNG_MIN, _LNG_MAX = 78.4, 79.0
...
async def main():
    ...
    print(f"Zepto Trichy Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(TRICHY_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(TRICHY_PINCODES)
        for i, (pincode, label) in enumerate(TRICHY_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Trichy ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_trichy_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Trichy — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Trichy — Store Count by City", generated)
