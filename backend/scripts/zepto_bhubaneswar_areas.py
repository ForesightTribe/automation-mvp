"""
Scrape Zepto dark stores for all Bhubaneswar pincodes.
...
Run from backend/:
    python -m scripts.zepto_bhubaneswar_areas
Output: zepto_bhubaneswar_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import BHUBANESWAR_PINCODES
...
# Bhubaneswar bounding box — covers Bhubaneswar city + Samalkha + Israna + Madlauda + Bapoli + Sanauli
_LAT_MIN, _LAT_MAX = 20.1, 20.6
_LNG_MIN, _LNG_MAX = 85.5, 86.0
...
async def main():
    ...
    print(f"Zepto Bhubaneswar Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(BHUBANESWAR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(BHUBANESWAR_PINCODES)
        for i, (pincode, label) in enumerate(BHUBANESWAR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Bhubaneswar ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_bhubaneswar_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Bhubaneswar — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Bhubaneswar — Store Count by City", generated)
