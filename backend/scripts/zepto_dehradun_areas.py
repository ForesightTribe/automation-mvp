"""
Scrape Zepto dark stores for all Dehradun pincodes.
...
Run from backend/:
    python -m scripts.zepto_dehradun_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import DEHRADUN_PINCODES
...
async def main():
    ...
    print(f"Zepto Dehradun Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(DEHRADUN_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(DEHRADUN_PINCODES)
        for i, (pincode, label) in enumerate(DEHRADUN_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Dehradun ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_dehradun_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Dehradun — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Dehradun — Store Count by City", generated)
