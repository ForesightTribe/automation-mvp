"""
Scrape Zepto dark stores for all Prayagraj pincodes.
...
Run from backend/:
    python -m scripts.zepto_prayagraj_areas
Output: zepto_prayagraj_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import PRAYAGRAJ_PINCODES
...
# Prayagraj bounding box — covers Prayagraj + Pratapgarh + Fatehpur + Kaushambi
_LAT_MIN, _LAT_MAX = 24.8, 26.5
_LNG_MIN, _LNG_MAX = 80.5, 82.5
...
async def main():
    ...
    print(f"Zepto Prayagraj Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(PRAYAGRAJ_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(PRAYAGRAJ_PINCODES)
        for i, (pincode, label) in enumerate(PRAYAGRAJ_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Prayagraj ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_prayagraj_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Prayagraj — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Prayagraj — Store Count by City", generated)
