"""
Scrape Zepto dark stores for all Kanpur pincodes.
...
Run from backend/:
    python -m scripts.zepto_kanpur_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import KANPUR_PINCODES
...
# Kanpur bounding box — covers Kanpur city + Kanpur Dehat + Unnao + Kannauj
_LAT_MIN, _LAT_MAX = 25.7, 27.2
_LNG_MIN, _LNG_MAX = 79.6, 81.0
...
async def main():
    ...
    print(f"Zepto Kanpur Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(KANPUR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(KANPUR_PINCODES)
        for i, (pincode, label) in enumerate(KANPUR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Kanpur ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_kanpur_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Kanpur — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Kanpur — Store Count by City", generated)
