"""
Scrape Zepto dark stores for all Kurukshetra pincodes.
...
Run from backend/:
    python -m scripts.zepto_kurukshetra_areas
Output: zepto_kurukshetra_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import KURUKSHETRA_PINCODES
...
# Kurukshetra bounding box — covers Kurukshetra + Thanesar + Pehowa + Shahabad + Ladwa
_LAT_MIN, _LAT_MAX = 29.7, 30.3
_LNG_MIN, _LNG_MAX = 76.5, 77.2
...
async def main():
    ...
    print(f"Zepto Kurukshetra Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(KURUKSHETRA_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(KURUKSHETRA_PINCODES)
        for i, (pincode, label) in enumerate(KURUKSHETRA_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Kurukshetra ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_kurukshetra_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Kurukshetra — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Kurukshetra — Store Count by City", generated)
