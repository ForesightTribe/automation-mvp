"""
Scrape Zepto dark stores for all Sonipat pincodes.
...
Run from backend/:
    python -m scripts.zepto_sonipat_areas
Output: zepto_sonipat_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import SONIPAT_PINCODES
...
# Sonipat bounding box — covers Sonipat + Gohana + Gannaur + Kharkhoda + Bahadurgarh
_LAT_MIN, _LAT_MAX = 28.7, 29.5
_LNG_MIN, _LNG_MAX = 76.6, 77.3
...
async def main():
    ...
    print(f"Zepto Sonipat Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(SONIPAT_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(SONIPAT_PINCODES)
        for i, (pincode, label) in enumerate(SONIPAT_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Sonipat ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_sonipat_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Sonipat — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Sonipat — Store Count by City", generated)
