"""
Scrape Zepto dark stores for all Bareilly pincodes.
...
Run from backend/:
    python -m scripts.zepto_bareilly_areas
Output: zepto_bareilly_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import BAREILLY_PINCODES
...
# Bareilly bounding box — covers Bareilly + Pilibhit + Badaun + Rampur border
_LAT_MIN, _LAT_MAX = 27.8, 28.8
_LNG_MIN, _LNG_MAX = 78.8, 80.5
...
async def main():
    ...
    print(f"Zepto Bareilly Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(BAREILLY_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(BAREILLY_PINCODES)
        for i, (pincode, label) in enumerate(BAREILLY_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Bareilly ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_bareilly_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Bareilly — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Bareilly — Store Count by City", generated)
