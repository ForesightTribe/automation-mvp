"""
Scrape Zepto dark stores for all Amritsar pincodes.
...
Run from backend/:
    python -m scripts.zepto_amritsar_areas
Output: zepto_amritsar_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import AMRITSAR_PINCODES
...
# Amritsar bounding box — covers Amritsar city + Tarn Taran + Ajnala + Attari
_LAT_MIN, _LAT_MAX = 31.4, 32.0
_LNG_MIN, _LNG_MAX = 74.6, 75.2
...
async def main():
    ...
    print(f"Zepto Amritsar Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(AMRITSAR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(AMRITSAR_PINCODES)
        for i, (pincode, label) in enumerate(AMRITSAR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Amritsar ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_amritsar_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Amritsar — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Amritsar — Store Count by City", generated)
