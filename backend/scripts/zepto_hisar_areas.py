"""
Scrape Zepto dark stores for all Hisar pincodes.
...
Run from backend/:
    python -m scripts.zepto_hisar_areas
Output: zepto_hisar_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import HISAR_PINCODES
...
# Hisar bounding box — covers Hisar + Hansi + Fatehabad + Narnaund + Barwala + Adampur
_LAT_MIN, _LAT_MAX = 28.5, 30.0
...
async def main():
    ...
    print(f"Zepto Hisar Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(HISAR_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(HISAR_PINCODES)
        for i, (pincode, label) in enumerate(HISAR_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Hisar ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_hisar_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Hisar — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Hisar — Store Count by City", generated)
