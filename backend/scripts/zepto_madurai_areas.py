"""
Scrape Zepto dark stores for all Madurai pincodes.
...
Run from backend/:
    python -m scripts.zepto_madurai_areas
Output: zepto_madurai_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import MADURAI_PINCODES
...
# Madurai bounding box — covers Madurai + Dindigul + Sivaganga + Kodaikanal
_LAT_MIN, _LAT_MAX = 9.5, 10.5
_LNG_MIN, _LNG_MAX = 77.4, 78.4
...
async def main():
    ...
    print(f"Zepto Madurai Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(MADURAI_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(MADURAI_PINCODES)
        for i, (pincode, label) in enumerate(MADURAI_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Madurai ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_madurai_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Madurai — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Madurai — Store Count by City", generated)
