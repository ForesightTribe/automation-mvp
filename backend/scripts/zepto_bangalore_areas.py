"""
Scrape Zepto dark stores for all Bangalore pincodes.
...
Run from backend/:
    python -m scripts.zepto_bangalore_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import BANGALORE_PINCODES
...
# Bangalore bounding box — rejects coordinates outside Bangalore
_LAT_MIN, _LAT_MAX = 12.7, 13.2
_LNG_MIN, _LNG_MAX = 77.4, 77.9
...
async def main():
    ...
    print(f"Zepto Bangalore Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(BANGALORE_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(BANGALORE_PINCODES)
        for i, (pincode, label) in enumerate(BANGALORE_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Bangalore ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_bangalore_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Bangalore — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Bangalore — Store Count by City", generated)
