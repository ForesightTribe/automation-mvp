"""
Scrape Zepto dark stores for all Vijayawada pincodes.
...
Run from backend/:
    python -m scripts.zepto_vijayawada_areas
Output: zepto_vijayawada_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import VIJAYAWADA_PINCODES
...
# Vijayawada bounding box — covers Vijayawada + Gannavaram + Penamaluru + Ibrahimpatnam
_LAT_MIN, _LAT_MAX = 15.8, 17.2
_LNG_MIN, _LNG_MAX = 80.0, 81.5
...
async def main():
    ...
    print(f"Zepto Vijayawada Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(VIJAYAWADA_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(VIJAYAWADA_PINCODES)
        for i, (pincode, label) in enumerate(VIJAYAWADA_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Vijayawada ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_vijayawada_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Vijayawada — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Vijayawada — Store Count by City", generated)
