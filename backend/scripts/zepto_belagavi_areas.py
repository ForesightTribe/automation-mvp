"""
Scrape Zepto dark stores for all Belagavi pincodes.
...
Run from backend/:
    python -m scripts.zepto_belagavi_areas
Output: zepto_belagavi_stores_TIMESTAMP.xlsx
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import BELAGAVI_PINCODES
...
# Belagavi bounding box — covers Belagavi + Gokak + Athani + Chikkodi + Dharwad border
_LAT_MIN, _LAT_MAX = 15.0, 17.0
_LNG_MIN, _LNG_MAX = 74.0, 75.5
...
async def main():
    ...
    print(f"Zepto Belagavi Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(BELAGAVI_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(BELAGAVI_PINCODES)
        for i, (pincode, label) in enumerate(BELAGAVI_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Belagavi ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_belagavi_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Belagavi — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Belagavi — Store Count by City", generated)
