"""
Scrape Zepto dark stores for all Kolkata pincodes.
...
Run from backend/:
    python -m scripts.zepto_kolkata_areas
"""
import asyncio
import sys
...
from scraper.platforms.zepto.dark_store.endpoints import KOLKATA_PINCODES
...
# Kolkata bounding box — rejects coordinates outside Kolkata region
_LAT_MIN, _LAT_MAX = 22.2, 23.1
_LNG_MIN, _LNG_MAX = 87.9, 88.7
...
async def main():
    ...
    print(f"Zepto Kolkata Dark Store Scraper — {ts}")
    print(f"Pincodes: {len(KOLKATA_PINCODES)} | all suggestions | exact details-API coords")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        total = len(KOLKATA_PINCODES)
        for i, (pincode, label) in enumerate(KOLKATA_PINCODES.items(), 1):
            ...
            for pred in preds:
                ...
                if not (_LAT_MIN <= lat <= _LAT_MAX and _LNG_MIN <= lng <= _LNG_MAX):
                    print(f"  ✗ {name[:55]} → outside Kolkata ({lat:.3f},{lng:.3f}), skipped")
                    await asyncio.sleep(0.15)
                    continue
    ...
    out = Path(__file__).parent.parent / f"zepto_kolkata_stores_{ts}.xlsx"
    ...
    apply_heading(ws, "Zepto Dark Stores Kolkata — All Pincode Suggestions", generated)
    apply_heading(ws2, "Zepto Kolkata — Store Count by City", generated)
