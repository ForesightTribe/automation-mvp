...
_HEADERS = {**HEADERS_COMMON, "Referer": "https://blinkit.com/"}


async def fetch_store(pincode: str) -> dict | None:
    url = f"https://www.blinkit.com/api/v1/store?pincode={pincode}"
    try:
        ...
        logger.warning(f"Blinkit store lookup failed for pincode {pincode}: {e}")
    return None
